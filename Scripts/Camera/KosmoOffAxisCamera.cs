...
namespace Kosmo
{
    ...
    [ExecuteInEditMode]
    public class KosmoOffAxisCamera : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            if (!Application.isPlaying || updateInPlayMode) UpdateMatrices();
        }
        ...
        public void UpdateMatrices()
        {
            ...
            if (null != projectionScreen && null != cameraComponent)
            {
                Vector3 pa =
                   ...
                   new Vector3(-screenWidth / 2, -screenHeight / 2, 0.0f));
        ...
        void OnDrawGizmos()
        {
            ...
            if (showScreen)
            {
                ...
                verts[1] = projectionScreen.transform.TransformPoint(screenWidth / 2, screenHeight / 2, 0);
                verts[2] = projectionScreen.transform.TransformPoint(screenWidth / 2, -screenHeight / 2, 0);
                verts[3] = projectionScreen.transform.TransformPoint(-screenWidth / 2, -screenHeight / 2, 0);

                Handles.DrawSolidRectangleWithOutline(verts, new Color(1, 1, 1, 0.05f), new Color(1, 1, 1, .5f));

                /*
                float test = 0;
                 foreach (Vector3 posCube in verts)

                    Handles.ScaleValueHandle(test,
                       posCube,
                       Quaternion.identity,
                       1,
